...
import { createBoss, getFloorsSpeed, initFloors, resetFloors, setEnemyPercent, setFloorsSpeed } from "./floors"
import { initHouses, playHouseIntro } from "./houses"
import { initHud, playHudIntro, setInfo, setScore } from "./hud"
...
const clouds = createEntity(["clouds", , [
    [, {t: [, [90, -40], [-1.3, 1]], s: SPRITE_CLOUD, c: [1, 1, 1, .2]}],
    [, {t: [, [-70, -30], 1,], s: SPRITE_CLOUD, c: [1, 1, 1, .3]}],
    [, {t: [, [-100, 15], [1.2, .7]], s: SPRITE_CLOUD, c: [1, 1, 1, .7]}]
]])

let speedToken: TTimerToken
...
async function onStart() {
    ...
    resetFloors()
    const speed = -100
    await timer(0.5, (t) => setFloorsSpeed(t * speed))
    speedToken = [1]
    await timer(
        60,
        (t) => {
